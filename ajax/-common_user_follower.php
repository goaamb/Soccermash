...
function getMyFollowers()
{
	...
	if(isset($_SESSION["iSMuIdKey"])){
		...
		$sPhoto='<span class="subtitlesLC paddingLC">Followers</span>
				  <p class="onright paddingLC"></p>
				  <ul>';

		$sPhotoF='<span class="subtitlesLC paddingLC"><a href="#" onclick="JS_getMyFollowers(); return false;">Followers</a></span>
				  <p class="onright paddingLC"></p>
				  <ul>';
		...
		for($i=0;$i<sizeof($aFollowers['id']);$i++){
			if($aFollowers['id'][$i]!=0){
				...
				$sPhotoF.='<li>	<a href="" title="'.$aFollowers['name'][$i].'" onclick="JS_follower('.$aFollowers['id'][$i].'); return false;">		   	
...
function getAllFollower($iPagActual)
{
	...
	if( !isset($_SESSION["idUserVisiting"]) || $_SESSION["idUserVisiting"]==0 || $_SESSION["idUserVisiting"]==$_SESSION["iSMuIdKey"]){#esta activo el USER ppal!
		if(isset($_SESSION["iSMuIdKey"])){
			...
			$sPhoto='<span><a title="Close"  onclick="$('."'".'#modules'."'".').show();$('."'".'#results'."'".').fadeOut('."'".'slow'."'".'); $('."'".'#videoPlayer'."'".').hide();$('."'".'#wall'."'".').fadeIn();$('."'".'#wall'."'".').fadeIn();" href="javascript:;">Close: </a></span>
				<h2>Following: </h2>
			...
			foreach($aFolloweres as $aFollower){
				...
				$sPhoto.='<div class="itemResult" onclick="JS_follower('.$aFollower['id'].'); return false;"  onmouseover="subir('.$aFollower['id'].');" onmouseout="bajar('.$aFollower['id'].');">
		...
		if(isset($_SESSION["idUserVisiting"])){
			...
			$sPhoto='<span><a title="Close"   onclick="$('."'".'#modules'."'".').show();$('."'".'#results'."'".').fadeOut('."'".'slow'."'".'); $('."'".'#videoPlayer'."'".').hide();" href="javascript:;">Close: </a></span>
				<h2>Following: </h2>
				<ul id="condition"><li></li></ul>';
			foreach($aFolloweres as $aFollower){
				////Move the img to center thumb//////////
...
function setLatestPeople(){
		...
		if( !isset($_SESSION["idUserVisiting"]) || $_SESSION["idUserVisiting"]==0 ){#esta activo el USER ppal!
			...
			if(isset($_SESSION["iSMuIdKey"])){
				...
				$sPhoto='<p class="greyTitles paddingRC"><a href="javascript:;" onClick="JS_getAllFeatured(1);">Featured Profiles</a> </p>
				...
				foreach($aDestacados as $aDestacado){
					   ...
					   //////Move the img to center thumb//////////
					    ...
					    $sPhoto.='<li><div class="imgProfileBorder" style="overflow:hidden;">
							<a href="" style="'.$moveLeft.' '.$moveTop.'" title="'.utf8_encode($aDestacado['name']).'" onclick="JS_follower('.$aDestacado['id'].'); return false;">		   	   <img src="photoGeneral/small/small_'.$aDestacado['photo'].'"  />
					   		</a></div>
						       </li>';
				...
				$sPhoto='<p class="greyTitles paddingRC"><a href="javascript:;" onClick="JS_getAllLatestRegistered(1);">Latest Members</a> </p>
			  				<img class="posBreak" src="img/break.png" width="200" height="3" />    <ul>';
				foreach($aLastRegisteredes as $aLastRegistered){
					   ...
					   //////Move the img to center thumb//////////
					    ...
					    $sPhoto.='<li><div class="imgProfileBorder" style="overflow:hidden;">
							<a href="" style="'.$moveLeft.' '.$moveTop.'" title="'.utf8_encode($aLastRegistered['name']).'" onclick="JS_follower('.$aLastRegistered['id'].'); return false;">		
				...
				$sDivCant='<p class="onright paddingLC" original-title="">'.$iCantTotal.'<span original-title="" id="clickFllwr" onclick="JS_getMyFollowing(1); return false;" class=" paddingLC cursor">   View All</span> </p>';
					   ...
					   //////Move the img to center thumb//////////
					    ...
					    if($aImPhoto[1]>50){
					     ...
					     $moveTop='';
							...
							<a href="" style="'.$moveLeft.' '.$moveTop.'"   title="'.$aFollowerData['name'].'" onclick="JS_follower('.$aFollowers['id'][$i].'); return false;">		   	
					   			<img src="photoGeneral/small/small_'.$aFollowerData["photo"].'"  />
							</a></div>
							</li>  ';
					}
				...
				if(isset($_SESSION["idUserVisiting"])){
					...
					foreach($aDestacados as $aDestacado){
						   ...
						   //////Move the img to center thumb//////////
						    ...
						    $sPhoto.='<li><div class="imgProfileBorder" style="overflow:hidden;">
								<a href="" style="'.$moveLeft.' '.$moveTop.'" title="'.$aDestacado['name'].'" onclick="JS_follower('.$aDestacado['id'].'); return false;">		   	   <img src="photoGeneral/small/small_'.$aDestacado['photo'].'"  />
						   		</a></div>
							       </li>';
						#}
						#$i++;
					...
					$sPhoto='<p class="greyTitles paddingRC">
							<a href="javascript:;" onClick="JS_getAllLatestRegistered(1);">Latest Members </a></p>
				  				<img class="posBreak" src="img/break.png" width="200" height="3" />    <ul>';
					foreach($aLastRegisteredes as $aLastRegistered){
						...
						if($aFollowers['id'][$i]!=0 ){
						    ...
						    if($aImPhoto[1]>50){
						     ...
						     $moveTop='';
			                            ...
			                            $sPhotoF.='<li>
								<div class="imgProfileBorder" style="overflow:hidden;">
								<a href="" style="'.$moveLeft.' '.$moveTop.'"   title="'.utf8_encode($aFollowerData['name']).'" onclick="JS_follower('.$aFollowers['id'][$i].'); return false;">		   	
						   			<img src="photoGeneral/small/small_'.$aFollowerData["photo"].'"  />
								</a></div>
								</li>  ';
...
function getAccount(){
		...
		if(isset($_SESSION["iSMuIdKey"])){
			...
			$sForm='    <div id="closing" title="Close"></div> 
					...
					<h2 id="pdH2">Personal Data Saved</h2>
					  ...
					  <div>
					    ...
					    Check your E-mail, was sent your new data.</label>
...
function setEmailPrivacy($aForm){
	...
	$oRespuesta->assign("main", "innerHTML", '<h5>EMAIL NOTIFICATIONS</h5> <h4>Your email preferences were saved.</h4>');#msj de exito!
	return $oRespuesta;
}#setEmailPrivacy
		...
		if(isset($_SESSION["iSMuIdKey"])){
			...
			{		
					...
					else
					{
						  ...
						  {
				         $aErrores['email2'] = 'E-mail already exists';
			...
			if(!empty($aForm['newOldPass'])){
				...
				if($sPass!=md5($aForm['newOldPass'])){
					$aErrores['passwordUser'] ='Incorrect Old Password';
				}else{
					if (empty($aForm['newUserPass']))
					{
						$aErrores['passwordUser2'] ='New Password empty';
					}else{
						if($aForm['newUserPass']!=$aForm['repeatPass']){
							$aErrores['passwordUser3'] ='Different Password';
						}else{
						 	$bPass=true;	
						}
					}
			...
			if (sizeof($aErrores) > 0)
			{
				...
				$oRespuesta->assign("error-update", "innerHTML", 'Check the data fields ');
				$oRespuesta->script("$('#error-update').fadeIn();");
	
				if (isset($aErrores['email1']))
				{
	
					$oRespuesta->script("$('#emailNewUser').tipsy({ trigger: 'manual', gravity:'e', fallback:'E-mail invalid format'});");
				...
				if (isset($aErrores['email2']))
				{
					...
					$oRespuesta->script("$('#emailNewUser').tipsy({ trigger: 'manual', gravity:'e', fallback:'E-mail already exists'});");
					$oRespuesta->script("$('#emailNewUser').tipsy('show');");
				...
				{
					$oRespuesta->script("$('#repeatEmail').tipsy({ trigger: 'manual', gravity:'e', fallback:'E-mail is Different!'});");
				...
				{
					$oRespuesta->script("$('#newOldPass').tipsy({ trigger: 'manual', gravity:'e', fallback:'Incorrect Old Password'});");
					$oRespuesta->script("$('#newOldPass').tipsy('show');");
					unset($aErrores['passwordUser']);
				}	
				if (isset($aErrores['passwordUser2']))
				{
					$oRespuesta->script("$('#newUserPass').tipsy({ trigger: 'manual', gravity:'e', fallback:'New Password empty'});");
					$oRespuesta->script("$('#newUserPass').tipsy('show');");
					unset($aErrores['passwordUse2r']);
				}
				if (isset($aErrores['passwordUser3']))
				{
					$oRespuesta->script("$('#repeatPass').tipsy({ trigger: 'manual', gravity:'e', fallback:'Different Password '});");
						...
						if($bEmail){
							if($bPass){#si ademas cambio la pass envio los 2 new
								...
								if(sendEmailUpdate($iUserIdSM,$sNames,$aForm['emailNewUser'],$aForm['newUserPass'],$sMsj)){
									$oRespuesta->assign("main", "innerHTML", '<h5>ACCOUNT</h5> <h4>An email was sent with your new data! <div> Check your account email, '.$aForm['emailNewUser'].'</div> </h4>');#msj de exito!
								}
								...
								$oRespuesta->assign("main", "innerHTML", '<h5>ACCOUNT</h5> <h4>An email was sent with your new data!
				  <div> Check your account email, '.$aForm['emailNewUser'].'</div> </h4>');#msj de exito!
							...
							if(sendEmailUpdate($iUserIdSM,$sNames,$sEmail,$sPass,$sMsj)){
								$oRespuesta->assign("main", "innerHTML", '<h5>ACCOUNT</h5> <h4>An email was sent with your new data!
				  <div> Check your account email, '.$sEmail.'</div> </h4>');#msj de exito!
							}
						}
					
					
					}else{#no se pudo modificar, =>again el form
						$oRespuesta->assign("error-update", "innerHTML", 'Could not change the data. Please, try again. ');
						$oRespuesta->script("$('#error-update').fadeIn();");
					}
				}#$bEmail || $bPass
			}#else, no hubo error
...
function enviarMsj($aForm){
		...
		if(isset($_SESSION["iSMuIdKey"]) && isset($_SESSION["idUserVisiting"])){
			...
			if($bAddMsj=$oMsj->agregarMsj($iIdUserSend,$iIdUserRecived,$sMsj)){
				$oRespuesta->assign("msgSender", "innerHTML", '<h2 id="emailTitle">Sent Message!!<span id=""></span></h2>');
			}#agregarMsj
			else{
				$oRespuesta->assign("msgSender", "innerHTML", '<h2 id="emailTitle">Could not Send Message!!<span id=""></span></h2>');
			}			
		}
		...
		if(isset($_SESSION["iSMuIdKey"])){
			...
			if(QMail::add($tipo, $usuario, $asunto, $archivo, $aUsuario, $prioridad)){
				...
				if(QMail::add($tipo, $iUserIdSM, $asunto, $archivo, $aUsuario, $prioridad)){
					...
					$oRespuesta->assign("msjErrorReport", "innerHTML", '<h2 id="emailTitle">You have reported an error. We will evaluate the case.
					Thanks for helping to improve your network!	</h2>');
				}

			}else{#no se envio el email

				$oRespuesta->assign("msjErrorReport", "innerHTML", '<h2 id="emailTitle">Messagge not send. Please try again!</h2>');
...
function mostrarPaginadoPeople($totalRegistros,$paginaActual,$limiteRegistros,$iFunction){
			...
			if (($paginaActual - 1) > 0)
			{				
				$paginado .= '<span class="paginador"><a href="javascript:;" onclick="'. $sFuncion.'('.($paginaActual - 1).');">previous</a></span>';

			}
			if (($paginaActual + 1) <= $totalPaginas)
			{

				$paginado .= '<span class="paginador"><a href="javascript:;" onclick="'. $sFuncion.'('.($paginaActual + 1).');">more results </a></span>';
...
function getAllFeatured($iPagActual){
		...
		if(isset($_SESSION["iSMuIdKey"])){
			...
			#echo 'INI:'.$inicio;die();
			#traer los users solamente, q esten activos y completos: active=1 y complete=1
			$aDestacados = $SITE->getUsuarios(NULL, "id!='$iIdUSer'". " AND ". $iEstado. " AND ".$iComplete. " AND ".$iDestacado,'registerDate DESC Limit '.$inicio.','.$iCantSet);
			#$sPhoto= ' <script type="text/javascript"> $("#centralcolumn").css("min-height","1400px");</script>';

			$sPhoto='<span><a title="Close"  onclick="$('."'".'#modules'."'".').show();$('."'".'#results'."'".').fadeOut('."'".'slow'."'".');
			 $('."'".'#videoPlayer'."'".').hide();$('."'".'#wall'."'".').fadeIn();$('."'".'#wall'."'".').fadeIn();" href="javascript:;">Close: </a></span>
			...
			$sPhoto.='<div class="itemResult" onclick="JS_follower('.$aDestacado['id'].'); return false;"  onmouseover="subir('.$aDestacado['id'].');" onmouseout="bajar('.$aDestacado['id'].');">
				...
				<ul id="condition"><li></li></ul>';
					...
					foreach($aSugestedPeoples as  $aSugestedPeople){
								  ...
								  $ecDD  = 'BRD: '.$brd[2].'/'.$brd[1].'/'.$brd[0];
								    ...
								    $sPhoto.='<div class="itemResult" onclick="JS_follower('.$aSugestedPeople['id'].'); return false;" 
...
function getWhoIsOnline(){
		...
		if(isset($_SESSION["iSMuIdKey"])){
			...
			if(sizeof($aOnlineId)>0){#existen user online con mis following
				...
				#write html
				$sPhotoOnline='<p class="greyTitles paddingRC"><a href="javascript:;" onClick="JS_getAllOnline(1);">Who\'s online</a> </p>
					...
					if($i<9){
						   //////Move the img to center thumb//////////
						    ...
						    $sPhotoOnline.='<li><div class="imgProfileBorder" style="overflow:hidden;">
								<a href="" style="'.$moveLeft.' '.$moveTop.'" title="'.$aOnline['name'].'" onclick="JS_follower('.$aOnline['id'].'); return false;">		   	   <img src="photoGeneral/small/small_'.$aOnline['photo'].'"  />
						   		</a></div>
							       </li>';
						}
						$i++;
							 ...
							 if (($tiempo >= $antes) && ($tiempo <= $ahora)){#tiempoUtlimaActividad
								...
								$id=$aOtherOnline['id'];
				 				...
				 				if($aStatusUser['status']!="invisible"){  
								    ...
								    $sPhotoOnline.='<li><div class="imgProfileBorder" style="overflow:hidden;">
										<a href="" style="'.$moveLeft.' '.$moveTop.'" title="'.$aOtherOnline['name'].'" onclick="JS_follower('.$aOtherOnline['id'].'); return false;">		   	   <img src="photoGeneral/small/small_'.$aOtherOnline['photo'].'"  />
								   		</a></div>
									       </li>';
								    $i++;
				 				}#$aStatusUser
							    ...
							    $id=$aOtherOnline['id'];
				 			    ...
				 			    if($aStatusUser['status']!="invisible"){ 	  
								  ...
								  $ecDD  = 'BRD: '.$brd[2].'/'.$brd[1].'/'.$brd[0];
								    ...
								    $sPhoto.='<div class="itemResult" onclick="JS_follower('.$aOtherOnline['id'].'); return false;" 
...
function getAllLatestRegistered($iPagActual){
		...
		if(isset($_SESSION["iSMuIdKey"])){
			...
			$sPhoto.='<div class="itemResult" onclick="JS_follower('.$aLastRegistered['id'].'); return false;"  onmouseover="subir('.$aLastRegistered['id'].');" onmouseout="bajar('.$aLastRegistered['id'].');">
