...
#Nueva function de add friend & send email friend notification
function addFollower(){
	...
	if(isset($_SESSION["iSMuIdKey"])){
		...
		if(isset($_SESSION["idUserVisiting"])){
			...
			if($iIdUser!=$iIdFollower){#si el ID de user es != al visit
					...
					$bFollower=$Follower->agregarFollower($iIdUser,$iIdFollower);#add friend
					$bFollower=$Follower->addFriendNotification($iIdUser,$iIdFollower);#add friend notification 
					
					if($bFollower){#send email friend notification
					     ...
					     $prioridad	= "Usuario";
					     $asunto=$aUserAccepted['name'].': '.$aUserAccepted['nameAccepted'].' {added you as your friend at SOCCERMASH.com}';
					     $tipo		= "addFriend";
					     $archivo	= '/templatemail/emailFriendRequest.tpl';
						QMail::add($tipo, $iIdFollower, $asunto, $archivo, $aUserAccepted, $prioridad);
					}#if($bFollower)
			}#$iIdUser!=$iIdFollower
			...
			/*$i=0;
			while($i<$iCantTotal){
					if(isset($aFolloweres['id'][$i]) && $iIdFollower==$aFolloweres['id'][$i]){
			...
			//var_dump($you);
			//var_dump(array_search($iIdFollower, $aFolloweres['id']));
			if(array_search($iIdFollower, $aFolloweres['id'])!==false){
			...
			#set the last Friend
			$aFollowers=$Follower->getHistoryFollowing($iIdFollower);#todos los  friends
				
			#Cant de friends
			$iCantTotal=count($aFollowers['id']);
			$aFollowers=$Follower->getAllFollowing($iIdFollower);
	...
	return $oRespuesta;#si falla send email
}#addFollower
...
function removeFollower(){
	...
	if(isset($_SESSION["iSMuIdKey"])){
		...
		if(isset($_SESSION["idUserVisiting"])){
			...
			if($iIdUser!=$iIdFollower){#si el ID de user es != al visit
				$bFollower=$Follower->removeFollower($iIdUser,$iIdFollower);#remove friend
			}#$iIdUser!=$iIdFollower
...
/*
# remove friend of inbox notification, "idUserVisiting" is not set
function removeFriend($iId){
	$oRespuesta = new xajaxResponse();
	...
	if(isset($_SESSION["iSMuIdKey"])){
		...
		if(isset($iId)){
			...
			#set 9 last Friend
			$aFollowers=$Follower->getHistoryFollowing($iIdUser);#todos los  MY friends
				
			#Cant de friends
			$iCantTotal=count($aFollowers['id']);
			$aFollowers=$Follower->getAllFollowing($iIdUser);
			$sDivCant='<p class="onright paddingLC" original-title="">'.$iCantTotal.' <span original-title="" id="clickFllwr" onclick="JS_getMyFollowing(1); return false;" class=" paddingLC cursor">   '.$_IDIOMA->traducir("View All").'</span> </p>';
			$sPhotoF=' <ul>';
...
function deleteInvitation($iIdFollower){
	
	$oRespuesta = new xajaxResponse();
	if(isset($_SESSION["iSMuIdKey"])){
		$iIdUser=(int)$_SESSION["iSMuIdKey"];
		$iIdFollower=(int)$iIdFollower;
		if($iIdUser!=$iIdFollower){#si el ID de user es != al visit
			global  $Follower;
			$Follower = new PeopleNet();	
			$bFollower=$Follower->deleteInvitation($iIdUser,$iIdFollower);
		}
		
		$iNewInvitation=$Follower->countInvitation($iIdUser);
		if($iNewInvitation>0){
			$oRespuesta->assign("numReceivedMsgFollow", "innerHTML", $iNewInvitation);#reescribo la cant de invitation friend
		}else{
			$oRespuesta->assign("numReceivedMsgFollow", "innerHTML", '');
			$oRespuesta->call('reOpenRequestCero');#saca la class del globo red
		}
		$oRespuesta->call('reOpenRequest');#funcion reload msj inbox invitation, -->tprgtmenu.php
			...
			if(isset($_SESSION["iSMuIdKey"])){
				...
				foreach($aLastRegisteredes as $aLastRegistered){
					   ...
					   //////Move the img to center thumb//////////
					    ...
					    $sPhoto.='<li><div class="imgProfileBorder" style="overflow:hidden;">
							<a href="/'.$_IDIOMA->traducir("user")."/".$aLastRegistered["id"]."-".Utilidades::normalizarTexto($aLastRegistered["name"]." ".$aLastRegistered["lastName"]).'" style="'.$moveLeft.' '.$moveTop.'" title="'.($aLastRegistered['name']).'">		
				...
				if(isset($_SESSION["idUserVisiting"])){
					...
					for($i=0;$i<sizeof($aFollowers['id']);$i++){
						if(isset($aFollowers['id'][$i])&& $aFollowers['id'][$i]!=0 ){
						    $idF=$aFollowers['id'][$i];
						    $aFollowerData = $SITE->getUsuario(NULL, "id='$idF'");	
						    $aImPhoto=array();
						    $aImPhoto=@getimagesize('../photoGeneral/small/small_'.$aFollowerData["photo"]);
...
#Name & last name
			$bChangeName=false;
			if (!empty($aForm['fname']) && ( !empty($aForm['lname'])  ) )
			{
				if($aForm['fname']!=$sName || $aForm['lname']!=$sLastName ){#si cambio name o ape
					$bChangeName=true;
				}
			}
			...
			#change name
			if($bChangeName){
					$aUpdate['name']=$aForm['fname'];
					$aUpdate['lastName']=$aForm['lname'];
					$aUserUdpate = $SITE->modificarUsuario($iUserIdSM, $aUpdate);
			}
...
#change name
				...
				if($bEmail || $bPass){
					...
					if($aUserUdpate = $SITE->modificarUsuario($iUserIdSM, $aUpdate)){
						...
						$sMsj=$_IDIOMA->traducir( 'Your New Personal Data for login:');
						if($bEmail){
							if($bPass){#si ademas cambio la pass envio los 2 new
								...
								if(sendEmailUpdate($iUserIdSM,$sNames,$aForm['emailNewUser'],$aForm['newUserPass'],$sMsj)){
									$oRespuesta->assign("main", "innerHTML", '<h5>'.$_IDIOMA->traducir("ACCOUNT").'</h5> <h4>'.$_IDIOMA->traducir("An email was sent with your new data").'!</h4>');#msj de exito!
								}
...
function mostrarPaginadoPeople($totalRegistros,$paginaActual,$limiteRegistros,$iFunction){
			...
			$paginado='<div class="pagRepresented">';
			if (($paginaActual - 1) > 0)
			{			
				$paginado .= '<a class="fl" href="javascript:;" onclick="'. $sFuncion.'('.($paginaActual - 1).');">&lt;</a>';	
				//$paginado .= '<span class="paginador"><a href="javascript:;" onclick="'. $sFuncion.'('.($paginaActual - 1).');">'.$_IDIOMA->traducir("previous").'</a></span>';

			}
			if ($totalPaginas>1) {
				for ($index = 1; $index <= $totalPaginas; $index++) {
					$paginado .= '<a class="num';
					if ($index==$paginaActual) {
						$paginado .=' active';
					} 
					$paginado .='" href="javascript:;" onclick="'. $sFuncion.'('.($index).');">'.$index.'</a>';
				}
			}
...
function mostrarPaginadoPeople($totalRegistros,$paginaActual,$limiteRegistros,$iFunction){
						...
						$sFuncion='JS_getAllOnline';#get All online
			  			...
			  			if($iFunction==5){
				  			...
				  			if($iFunction==6){
				  				...
				  				if($iFunction==8){
				  				$sFuncion='JS_mostrarPlayers';#get represented player
					  			}else	
				  					$sFuncion='JS_representedPlayers';#get represented player
			  				}
			  			}
			  		}
			  	}
			...
			if($iFunction!=7 && $iFunction!=8){#sino es el paginado de Representantes, tiene diferentes estilos
				if (($paginaActual - 1) > 0)
				{				
					$paginado .= '<span class="paginador"><a href="javascript:;" onclick="'. $sFuncion.'('.($paginaActual - 1).');">'.$_IDIOMA->traducir("previous").'</a></span>';
			...
			}else{#es el paginado de Representantes, tiene diferentes estilos
				if (($paginaActual - 1) > 0)
				{				
					$paginado .= '<a class="fl" href="javascript:;" onclick="'. $sFuncion.'('.($paginaActual - 1).');">&lt;</a>';
					#<a class="fl">&lt;</a>
...
function getAllFeatured($iPagActual){
		...
		if(isset($_SESSION["iSMuIdKey"])){
			...
			$iCantTotal=sizeof($SITE->getUsuarios(NULL, " ". $iEstado. " AND ".$iComplete. " AND ".$iDestacado));#cant total Destac
			$iCantSet=15;#cant de record x page
			...
			#echo 'INI:'.$inicio;die();
			#traer los users solamente, q esten activos y completos: active=1 y complete=1
			$aDestacados = $SITE->getUsuarios(NULL, " ". $iEstado. " AND ".$iComplete. " AND ".$iDestacado,'fecha_destacado desc,registerDate DESC Limit '.$inicio.','.$iCantSet);
...
#send a represented notification for user(Ej: player)
function addRepresentedNotifi(){
	...
	if(isset($_SESSION["iSMuIdKey"])){
			...
			if(isset($_SESSION["idUserVisiting"])){
				$iIdInvited		= (int)$_SESSION["idUserVisiting"];
				$bRepresented	= $SITE->sendRepresented($iIdUser, $iIdInvited);
				#Send Email
				#1. id de usuario de soccermash
				#2. * para todos los usuarios (con verificacion de permisos)
				#3. algo@soccermash.com emails terminados en soccermash otros no se acepta
				...
				$aUserRecived['perfilAgent'] 		= nameProfile($aUsuario['profileId']);
				
				$prioridad					= "Sistema";
				$asunto						= '{You have a Representation request in SOCCERMASH.com}';
				$tipo						= "alguno";
				$archivo					= '/templatemail/emailSendRepresented.tpl';
	...
	return $oRespuesta;
}#addRepresentedNotifi

#delete a represented agent
function removeNotiAgent($iId){
	$oRespuesta = new xajaxResponse();
	if(isset($_SESSION["iSMuIdKey"])){
			global   		$SITE;
			$SITE   		= new SITE();
			...
			$iIdUser		= (int)$_SESSION["iSMuIdKey"];
			$iId			= (int)$iId;
			$bRepresented	= $SITE->deleteRepresentedNotification($iIdUser, $iId);
	}		
	return $oRespuesta;
...
function addAgent($iIdAgent){
	...
	if(isset($_SESSION["iSMuIdKey"])){
			...
			$aUsuario		= Array();		
			$iIdUser		= (int)$_SESSION["iSMuIdKey"];
			$bRepresented	= $SITE->addAgent($iIdUser, $iIdAgent);
			
			$aUsuario	 	= $SITE->getUsuario(NULL, "id='$iIdUser'");
			$iIdAgent		= (int)$iIdAgent;
			#Send Email
			#1. id de usuario de soccermash
			#2. * para todos los usuarios (con verificacion de permisos)
			#3. algo@soccermash.com emails terminados en soccermash otros no se acepta

			$aUserRecived 				= $SITE->getUsuario(NULL, "id='$iIdAgent'");
			$aUserRecived['namePlayer'] 	= $aUsuario['name'].' '.$aUsuario['lastName'];
				
			$prioridad					= "Sistema";
			$asunto						= '{Your Representation Request has been accepted in SOCCERMASH.com
			m}';
			$tipo						= "alguno";
			$archivo					= '/templatemail/emailAddRepresent.tpl';
				
			if(QMail::add($tipo, $iIdAgent, $asunto, $archivo, $aUserRecived, $prioridad)){
					$b=true;;
			}			
...
function deleteAgent($iIdAgent){
	...
	if(isset($_SESSION["iSMuIdKey"])){
			...
			$iIdUser		= (int)$_SESSION["iSMuIdKey"];
			$iIdAgent		= (int)$iIdAgent;
			$bRepresented	= $SITE->deleteAgent($iIdUser);
...
#Mostar Player Representados p cualkier perfil
function mostrarPlayers($iPagActual){
	global $_IDIOMA;
	...
	if(isset($_SESSION["idUserVisiting"])){
			...
			#represented_users
			global   		$SITE;
			$SITE   		= new SITE();$oProfile= new Profile();
			$aUsuario		= Array();		
			$iIdAgent		= (int)$_SESSION["idUserVisiting"];

			#get Array players representeds by agent
			$aRepresenteds	= $SITE->getUsersRepresented($iIdAgent);	
			$iEstado    	='active=1';$iComplete   ='complete=1';
			#Paginator
			if(isset($iPagActual)){ $page=$iPagActual;	}else $page=1;			
			$iCantTotal	= sizeof($aRepresenteds);#cant total de player representd
			$iCantSet=6;#cant de record x page
			$array 		= mostrarPaginadoPeople($iCantTotal,$page,$iCantSet,8); #$iCantTotal:cant Total;$page:pag actual;$cant:record visibles;4:tipo de Func
			list($paginado, $inicio) = $array; 
			$sSqlId    	=  implode("','", $aRepresenteds);
			$aPlayers  	=  $SITE->getUsuarios(NULL, "id IN ('$sSqlId')". " AND ". $iEstado. " AND ".$iComplete,'registerDate DESC Limit '.$inicio.','.$iCantSet);
			$sPhoto='';
			foreach($aPlayers as $aPlayer){
					//////Move the img to center thumb//////////
					$aImPhoto=array();
					$aImPhoto=@getimagesize('../photoGeneral/big/'.$aPlayer['photo']);
							    
					if($aImPhoto[0]>180){ $moveLeft='style="margin-left:-'.(($aImPhoto[0]-180)/2).'px;"'; }else{ $moveLeft='';}
					#dayOfBirthDay
					if(!empty($aPlayer['dayOfBirthDay'])){
						   $brd=explode('-',$aPlayer['dayOfBirthDay']);
						   $sEdad='('.edad($aPlayer['dayOfBirthDay']).')';
					}else{ $brd[0]=''; $brd[1]=''; $brd[2]=''; }
						
					#Get Data Profesional
					if($aPlayer['profileId']<7 && $aPlayer['profileId']>1){ #si es perfil player
		        			$aJugador=$oProfile->selectProfile(2,'clubName,otherClub,position,endingContractDate',"idUser=".$aPlayer['id'].""); 
							if($aPlayer['profileId']==2){
							   	$ecDD='ECD: '.$aJugador[0]->endingContractDate;
							 }else{ $ecDD  = 'BRD: '.$brd[2].'/'.$brd[1].'/'.$brd[0];}
		
							 if(!empty($aJugador[0]->clubName)){#name club
								$sNameClub=$aJugador[0]->clubName;					 
							 }else{
								 if(!empty($aJugador[0]->otherClub))
										$sNameClub=$aJugador[0]->otherClub;
								 else   $sNameClub='-';
							 }
							 if(!empty($aJugador[0]->position))#position
							 		$posP  = namePosition($aJugador[0]->position);
							 else   $posP  = '-';
							
							 $clubB = 'CLUB: '.$sNameClub;                  
					}
			
					$sPhoto.='<div class="represented" onclick="location.href=\'/'.$_IDIOMA->traducir("user")."/".$aPlayer["id"]."-".Utilidades::normalizarTexto($aPlayer["name"]." ".$aPlayer["lastName"]).'\'" style="cursor:pointer;">
								<div class="image">
									<img '.$moveLeft.' src="photoGeneral/big/photoPerfil_SM_'.$aPlayer['id'].'">
								</div>
								
							    <ul>
									<li class="nam">'.$aPlayer['name'].' '.$aPlayer['lastName'].' '.$sEdad.'</li>
									<li>'.nameProfile($aPlayer['profileId']).'</li>
									<li>'.$ecDD.' </li>
									<li>'.$posP.'</li>
									<li>'.$aPlayer['countryName'].'</li>
									<li>'.$clubB.'</li>
								</ul>
							</div>	
							';
			...
			#Falta asignar el paginador
			$oRespuesta->assign("contentRepresented", "innerHTML", $sPhoto);
			$oRespuesta->assign("paginadorAgente", "innerHTML", $paginado);# paginador 
	}
...
#Get player representeds for Agent
function representedPlayers($iPagActual){
	$oRespuesta = new xajaxResponse();
	if(isset($_SESSION["iSMuIdKey"])){
		if(isset($_SESSION["isAgent"]) && ($_SESSION["isAgent"]==1) ){#es un agente
			#represented_users
			global   		$SITE;global $_IDIOMA;
			$SITE   		= new SITE();$oProfile= new Profile();
			$aUsuario		= Array();		
			$iIdAgent	= (int)$_SESSION["iSMuIdKey"];
			...
			$iCantTotal	= sizeof($aRepresenteds);#cant total de player representd
			$iCantSet=6;#cant de record x page
			$array 		= mostrarPaginadoPeople($iCantTotal,$page,$iCantSet,7); #$iCantTotal:cant Total;$page:pag actual;$cant:record visibles;4:tipo de Func
			list($paginado, $inicio) = $array; 
			$sSqlId    	=  implode("','", $aRepresenteds);
			$aPlayers  	=  $SITE->getUsuarios(NULL, "id IN ('$sSqlId')". " AND ". $iEstado. " AND ".$iComplete,'registerDate DESC Limit '.$inicio.','.$iCantSet);
			...
			foreach($aPlayers as $aPlayer){
					...
					$sPhoto.='<div class="represented">
								<div class="image">
									<a class="delRepresented" id="del'.$aPlayer['id'].'" href="javascript:;" onmouseover="$(\'#del'.$aPlayer['id'].'\').addClass(\'delRepresentedS\');" 
									   onmouseout="$(\'#del'.$aPlayer['id'].'\').removeClass(\'delRepresentedS\');" onclick="deletePlayer('.$aPlayer['id'].');">'.$_IDIOMA->traducir("remove").'</a>


									<img '.$moveLeft.' src="photoGeneral/big/photoPerfil_SM_'.$aPlayer['id'].'">
										<a class="admRepresented" id="adm'.$aPlayer['id'].'" href="javascript:;"
onmouseover="$(\'#adm'.$aPlayer['id'].'\').addClass(\'admRepresentedS\');"										onmouseout="$(\'#adm'.$aPlayer['id'].'\').removeClass(\'admRepresentedS\');" onclick="adminPlayer(\'/usuario/'.$aPlayer['id'].'-'.str_replace(" ","_",$aPlayer['name']).'_'.str_replace(" ","_",$aPlayer['lastName']).'\',\''.str_replace(" ","%20",$aPlayer['name']).'\',\''.str_replace(" ","%20",$aPlayer['lastName']).'\');">'.$_IDIOMA->traducir("Administrate profile").'</a>
								</div>   
...
# delete a player represented by agent
function  deletePlayerRepresented($iIdPlayer){
	$oRespuesta = new xajaxResponse();
	if(isset($_SESSION["iSMuIdKey"])){
			global   		$SITE;
			$SITE   		= new SITE();
			$aUsuario		= Array();		
			$iIdAgent		= (int)$_SESSION["iSMuIdKey"];
			$iIdPlayer		= (int)$iIdPlayer;
			$bDelete	= $SITE->deleteRepresentedUser($iIdAgent,$iIdPlayer);	
			$oRespuesta->call('JS_representedPlayers');
	}	
	return $oRespuesta;
}#deletePlayerRepresented
    #instances
...
$oXajaxFollower->registerFunction("addRepresentedNotifi");#new 
	$oXajaxFollower->registerFunction("removeNotiAgent");#new
	$oXajaxFollower->registerFunction("addAgent");#new   
	$oXajaxFollower->registerFunction("deleteAgent");#new 
	$oXajaxFollower->registerFunction("representedPlayers");#new  
	$oXajaxFollower->registerFunction("deletePlayerRepresented");#new  
	$oXajaxFollower->registerFunction("mostrarPlayers");#new  
