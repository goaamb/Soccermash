...
foreach ($iphone_language as $i => $l) {
	$iphone_language[$i] = str_replace("'", "\'", $l);
}
				...
				return {
					...
					updateBuddyList: function(data) {
						...
						$.each(data, function(i,buddy) {
							...
							buddylist += '<li class="onlinelist" id="onlinelist_'+buddy.id+'" onclick="javascript:jqcc.iphone.loadPanel(\''+buddy.id+'\')"><img src="'+buddy.a+'" class="avatarimage">'+longname+'<div class="status">'+buddy.s+'</div><div class="newmessages"></div></li>';
					...
					loggedOut: function() {
						alert('<?php echo $iphone_language[5];?>');
						location.href = jqcc.cometchat.getBaseUrl()+'../';
					},
					...
					sendMessage: function(id) {
						...
						fromname = '<?php echo $iphone_language[6];?>';
						selfstyle = 'selfmessage';

						var ts = Math.round(new Date().getTime() / 1000)+''+Math.floor(Math.random()*1000000);

						var temp = (('<li><div class="cometchat_chatboxmessage '+selfstyle+'" id="cometchat_message_'+ts+'"><span class="cometchat_chatboxmessagefrom"><strong>'+fromname+'</strong><?php echo $iphone_language[7];?></span><span class="cometchat_chatboxmessagecontent">'+message+'</span>'+'</div></li>'));
					...
					newMessage: function(incoming) {
						...
						if (incoming.self == 0) {
							var temp = (('<li><div class="cometchat_chatboxmessage" id="cometchat_message_'+ts+'"><span class="cometchat_chatboxmessagefrom"><strong>'+fromname+'</strong><?php echo $iphone_language[7];?></span><span class="cometchat_chatboxmessagecontent">'+incoming.message+'</span>'+'</div>'));
						...
						$.each(data, function(type,item){
								if (type == 'messages') {
									...
									$.each(item, function(i,incoming) {
										...
										temp += ('<li><div class="cometchat_chatboxmessage '+selfstyle+'" id="cometchat_message_'+incoming.id+'"><span class="cometchat_chatboxmessagefrom'+selfstyle+'"><strong>'+fromname+'</strong><?php echo $iphone_language[7];?></span><span class="cometchat_chatboxmessagecontent'+selfstyle+'">'+incoming.message+'</span>'+'</div>');
