<?php
	...
	<span class="clear pos" id="theCityName"><label class="posLabel" for="'.$this->name.'"><div style="margin-left:'.$this->margin.'px;">'.$this->title.'</div></label>
     ...
     <div id="'.$this->showIt.'" class="dontshow">
                  <ul id="'.$this->selector.'">
						...
						$registro = $bd->query("SELECT ID,NAME FROM ax_city WHERE CountryCode='".$this->CountryCode."' order by Name asc");
	  ...
	  <span class="clear pos" id="cityLink">
		  <a href="javascript:;" onclick="$(\'#theCityName\').hide(); $(\'#cityName\').val(\'-\'); $(\'#othCity\').show(); $(\'#cityLink\').hide();">'.$_IDIOMA->traducir("Other City").'</a>
          </span>
           	
		  <span class="clear pos" id="othCity" style="display:none;">
	  	 
          <label id="ooCity" for="otherCity">'.$_IDIOMA->traducir("Other City").'</label>
