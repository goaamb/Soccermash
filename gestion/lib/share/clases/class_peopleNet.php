...
*/
	...
	function __construct() {
		...
		$this->sFieldsFollower = 'id_user,follower,following,history_follower,history_following,follower_invitation'; #ax_follower
		$this->oDBLP = & $SITE_oDB;
	...
	*/
	...
	#Add Invitation
	function addFriendNotification($iIdUser, $iIdInvitation) {
		
		$aGetArray = Array ();
		$aVectorFollower = Array ();
		#get invitation
		$sSQL_Select = GenerateSelect ( $this->sFieldsFollower, SITE_DB_TB_SystemFollower, "id_user='$iIdInvitation'" );
		$DB_Result = $this->oDBLP->Query ( $sSQL_Select );
		$aGetArray = $this->oDBLP->FetchRowAssoc ( $DB_Result );
		$aInvitation = unserialize ( $aGetArray ['follower_invitation'] );
		
		#Add position al final del vector Id
		if (array_search ( $iIdUser, $aInvitation ) === false) {
			array_push ( $aInvitation, $iIdUser );
			$aInvitation = serialize ( $aInvitation );
			$aGetArray ['follower_invitation'] = $aInvitation;
			
			#Save New Invitation		
			$sSQL_Update = GenerateUpdate ( SITE_DB_TB_SystemFollower, $aGetArray, "id_user='$iIdInvitation'" );
			$bresultUpdate = $this->oDBLP->Query ( $sSQL_Update );
	...
	#Count Invitation
	function countInvitation($iIdUser) {
		$aGetArray = Array ();
		$aVectorFollower = Array ();
		#get invitation
		...
		if(is_array($aInvitation) && count($aInvitation)>0){
			$iCount = count ( $aInvitation );
			foreach ($aInvitation as $invitado) {
				$cuenta=$ml->contar("id='$invitado'");
				if($cuenta==0){
					$iCount--;
				}
			}
	...
	#Get New Invitation
	function getInvitation($iIdUser) {
		...
		$DB_Result = $this->oDBLP->Query ( $sSQL_Select );
		$aGetArray = $this->oDBLP->FetchRowAssoc ( $DB_Result );
		$aVectorFollower = unserialize ( $aGetArray ['follower_invitation'] );
		
		#retorna el vector de new invitation
	...
	function deleteInvitation($iIdUser, $iIdInvitation) {
		
		$aGetArray = Array ();
		$aVectorFollower = Array ();
		$bInvitation = false;
		#get array invitation
		$sSQL_Select = GenerateSelect ( $this->sFieldsFollower, SITE_DB_TB_SystemFollower, "id_user='$iIdUser'" );
		$DB_Result = $this->oDBLP->Query ( $sSQL_Select );
		$aGetArray = $this->oDBLP->FetchRowAssoc ( $DB_Result );
		$aInvitation = unserialize ( $aGetArray ['follower_invitation'] );
		
		for($i = 0; $i < count ( $aInvitation ); $i ++) {
			#if($aInvitation[$i]==$iIdInvitation){
			if (isset ( $aInvitation [$i] ) && $iIdInvitation == $aInvitation [$i]) {
				unset ( $aInvitation [$i] );
				$bInvitation = true;
			}
		}
		
		$aInvitation = array_values ( $aInvitation ); #reordena el array
		$aInvitation = serialize ( $aInvitation );
		$aGetArray ['follower_invitation'] = $aInvitation;
	...
	function getExistFollower($iIdUser, $iIdFollower) {
		...
		if (array_search ( $iIdFollower, $aHistoryIds ) === false) {
			$aHistoryIds [] = $iIdFollower;
		}
		$newAHistoryIds=array();
		foreach ($aHistoryIds as $numvalue) {
			if(array_search($numvalue, $newAHistoryIds)===false){
				$newAHistoryIds[]=$numvalue;
			}
		}
		$aHistoryIds=$newAHistoryIds;
		
		if (count ( $aHistoryIds ) > 9) {
			$aFollowerIds = array_slice ( $aHistoryIds, count ( $aHistoryIds) - 9 );
		}else{
			$aFollowerIds=array_values($aHistoryIds);
		}
		if (count ( $aFollowerIds ) < 9) {
			$countfr = (9 - count ( $aFollowerIds ));
			for($i = 0; $i < $countfr; $i ++) {
				$aFollowerIds [] = 0;
			}
		}
	...
	function agregarFollowing($iIdUser, $iIdFollowing) {
		...
		if (array_search ( $iIdUser, $aHistoryIds ) === false) {
			$aHistoryIds [] = $iIdUser;
		}
		$newAHistoryIds=array();
		foreach ($aHistoryIds as $numvalue) {
		...
		for($i = 0; $i < sizeof ( $aFollowingIds ); $i ++) {
			...
			if (isset ( $aFollowingIds ['id'] [$i] ) && $iIdUser == $aFollowingIds ['id'] [$i]) {
				$bExist = true;
			}
		}
		...
		if (! $bExist) { #no esta en las ultimos seguimientos
			...
			for($i = 0; $i < sizeof ( $aFollowingIds ); $i ++) {
				...
				if (isset ( $aFollowingIds ['id'] [$i] ) && 0 == $aFollowingIds ['id'] [$i] && ! $bPlace) {
		...
		for($j = 0; $j < sizeof ( $aHistoryIds ); $j ++) {
			...
			if (isset ( $aHistoryIds ['id'] [$j] ) && $iIdUser == $aFollowingIds ['id'] [$j]) {
	...
	*/
	...
	function removeFollower($iIdUser, $iIdFriend) {
		...
		$pos=array_search ( $iIdFriend, $aHistoryFollower ['id']);
		if($pos!==false){
			array_splice($aHistoryFollower ['id'], $pos,1);
		}
		
		if (count ( $aHistoryFollower ['id'] ) > 9) {
			$aArrayFollower = array("id"=>array_slice ( $aHistoryFollower ['id'], count ( $aHistoryFollower ['id']) - 9 ));
		}else{
			$aArrayFollower =array("id"=>array_values($aHistoryFollower ['id']));
		}
		if (count ( $aArrayFollower["id"] ) < 9) {
			$countfr = (9 - count ( $aArrayFollower["id"] ));
			for($i = 0; $i < $countfr; $i ++) {
				$aArrayFollower["id"] [] = 0;
			}
		}
		...
		#My Last 9		
		$aLast = Array (0, 0, 0, 0, 0, 0, 0, 0, 0 );
		$aHistoryIds = $aHistoryFollower ['id']; #get Array Ids of History Friends:Actualizados
		$iCantF = count ( $aHistoryIds );
		
		#new
		if ($iCantF > 0) { #tiene al menos 1 friend
			$j = 8;
			for($i = ($iCantF - 1); $i >= 0; $i --) {
				if ($j >= 0) {
					if (isset ( $aHistoryIds [$i] )) {
						$aLast [$j] = $aHistoryIds [$i];
						$j --;
					}
				} else {
					break;
				} #sale xq se termino el array Last 9
			} #for	
		

		} else { #no tiene friend
			$aLast = Array (276, 0, 0, 0, 0, 0, 0, 0, 0 );
		}*/
		...
		//$aArrayFollower ['id'] = $aLast; #last 9
		$aArrayFollower = serialize ( $aArrayFollower );
		$aGetArray ['follower'] = $aArrayFollower;
		...
		$pos=array_search ( $iIdUser, $aHistoryFollower ['id']);
		if($pos!==false){
			array_splice($aHistoryFollower ['id'], $pos,1);
		}
