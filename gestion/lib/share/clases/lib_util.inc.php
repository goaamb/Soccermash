...
function namePosition($iPosition) {
	...
	if ($iPosition == 1) {
		return $nNamePosition = $_IDIOMA->traducir ( "Goalkeeper" );
		break;
	}
	if ($iPosition == 2) {
		return $nNamePosition = $_IDIOMA->traducir ( "Left Back" );
		break;
	}
	if ($iPosition == 3) {
		return $nNamePosition = $_IDIOMA->traducir ( "Central Back Left" );
		break;
	}
	if ($iPosition == 4) {
		return $nNamePosition = $_IDIOMA->traducir ( "Central Back Right" );
		break;
	}
	if ($iPosition == 5) {
		return $nNamePosition = $_IDIOMA->traducir ( "Right Back" );
		break;
	}
	if ($iPosition == 6) {
		return $nNamePosition = $_IDIOMA->traducir ( "Defensive Midfielder" );
		break;
	}
	if ($iPosition == 7) {
		return $nNamePosition = $_IDIOMA->traducir ( "Creative Midfielder" );
		break;
	}
	if ($iPosition == 8) {
		return $nNamePosition = $_IDIOMA->traducir ( "Left Wing" );
		break;
	}
	
	if ($iPosition == 9) {
		return $nNamePosition = $_IDIOMA->traducir ( "Right Wing" );
		break;
	}
	if ($iPosition == 10) {
		return $nNamePosition = $_IDIOMA->traducir ( "Play Maker / Second Striker" );
		break;
	}
	if ($iPosition == 11) {
		return $nNamePosition = $_IDIOMA->traducir ( "Target Striker" );
		break;
	}
}
...
function ValidateUrl($sUrl) {
	$aExpReg = array ('protocol' => '((http|https|ftp)://)', 'access' => '(([a-z0-9_]+):([a-z0-9-_]*)@)?', 'sub_domain' => '(([a-z0-9_-]+\.)*)', 'domain' => '(([a-z0-9-]{2,})\.)', 'tld' => '(com|net|org|edu|gov|mil|int|arpa|aero|biz|coop|info|museum|name|ad|ae|af|ag|ai|al|am|an|ao|aq|ar|as|at|au|aw|az|ba|bb|bd|be|bf|bg|bh|bi|bj|bm|bn|bo|br|bs|bt|bv|bw|by|bz|ca|cc|cf|cd|cg|ch|ci|ck|cl|cm|cn|co|cr|cs|cu|cv|cx|cy|cz|de|dj|dk|dm|do|dz|ec|ee|eg|eh|er|es|et|fi|fj|fk|fm|fo|fr|fx|ga|gb|gd|ge|gf|gh|gi|gl|gm|gn|gp|gq|gr|gs|gt|gu|gw|gy|hk|hm|hn|hr|ht|hu|id|ie|il|in|io|iq|ir|is|it|jm|jo|jp|ke|kg|kh|ki|km|kn|kp|kr|kw|ky|kz|la|lb|lc|li|lk|lr|ls|lt|lu|lv|ly|ma|mc|md|mg|mh|mk|ml|mm|mn|mo|mp|mq|mr|ms|mt|mu|mv|mw|mx|my|mz|na|nc|ne|nf|ng|ni|nl|no|np|nr|nt|nu|nz|om|pa|pe|pf|pg|ph|pk|pl|pm|pn|pr|pt|pw|py|qa|re|ro|ru|rw|sa|sb|sc|sd|se|sg|sh|si|sj|sk|sl|sm|sn|so|sr|st|su|sv|sy|sz|tc|td|tf|tg|th|tj|tk|tm|tn|to|tp|tr|tt|tv|tw|tz|ua|ug|uk|um|us|uy|uz|va|vc|ve|vg|vi|vn|vu|wf|ws|ye|yt|yu|za|zm|zr|zw)', 'port' => '(:(\d+))?', 'path' => '((/[a-z0-9-_.%~]*)*)?', 'query' => '(\?[^? ]*)?' );
...
function ArrayMerge($aOne = array(), $aTwo = array(), $aThree = NULL, $aFour = NULL) {
	...
	foreach ( $aOne as $mKey => $mVal )
		$aResult [$mKey] = $mVal;
	foreach ( $aTwo as $mKey => $mVal )
		$aResult [$mKey] = $mVal;
	if (gettype ( $aThree ) == 'array') {
		foreach ( $aThree as $mKey => $mVal )
			$aResult [$mKey] = $mVal;
	}
	if (gettype ( $aFour ) == 'array') {
		foreach ( $aFour as $mKey => $mVal )
			$aResult [$mKey] = $mVal;
	}
	return $aResult;
} #ArrayMerge()
...
function GenerateUpdate($sTable = NULL, $aRow = array(), $sWhere = NULL, $bAplicarSlashes = true) {
	$aSet = array ();
	foreach ( $aRow as $sField => $sValue )
		$aSet [] = "$sField='" . ($bAplicarSlashes ? addslashes ( $sValue ) : $sValue) . "'";
...
function Array2Query($aData) {
	if (empty ( $aData ) or ! is_array ( $aData ))
		return false;
...
function dirty($id) {
	...
	switch ($pos) {
		case 1 :
			$asd = $id . $one . $two . $three;
			break;
		
		case 2 :
			$asd = $one . $id . $two . $three;
			break;
		
		case 3 :
			$asd = $one . $two . $id . $three;
			break;
		
		case 4 :
			$asd = $one . $two . $three . $id;
			break;
	}
	$dirtyParam = $pos . $asd;
	return $dirtyParam;
