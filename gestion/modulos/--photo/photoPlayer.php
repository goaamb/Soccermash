...
require_once($_SERVER['DOCUMENT_ROOT'].$dir.'/gestion/modulos/photo/photoClass.php');
				...
				$editProfile=addslashes(base64_decode($iFv[10]));
				...
				<div class="onright"><a href="javascript:;" onclick="hide_me('videoPlayer'); $('#modules').css('margin-top','0px');">Close</a></div> 
				...
				if($sSiz[0]>588){
					...
					$widthTotal='';
					$moveRi=(589-$sSiz[0])/2;
					$moveRight='padding-left:'.$moveRi;		
					$imgTop=$sSiz[1]/2;
				}
				
				if($sSiz[1]>700 && $sSiz[0]<588){
					$heightTotal='width=\'700\'';
					$imgTop=700/2;
				}else{
					$heightTotal='';
				}
	...
	if($registros[0]!=''){
		...
		foreach($registros as $registro){
			...
			<script type="text/javascript">
				...
				if($ee<$i){
				?>
				<div style="position:absolute; z-index:3; top:<? echo $imgTop; ?>; left:576px;"><img onclick="loadPhoto(pFvpu_pOkuOk_n_ph_uN_pN<? echo $ee; ?>); <? if(isset($iFv[12])){ ?>upPhotoPlayer();<? } ?>"   src="img/rgtOff.png"/></div>
				<?
				}
				?>
				...
				if($aa>=0){ 
				?>
					<div style="position:absolute; left:556px; z-index:3; top:<? echo $imgTop; ?>;"><img onclick="loadPhoto(pFvpu_pOkuOk_n_ph_uN_pN<? echo $aa; ?>); <? if(isset($iFv[12])){ ?>upPhotoPlayer();<? } ?>" src="img/lftOff.png"/></div>
				<?
				}
				?>
				<div id="imgPhoto" style="position:relative; z-index:2; <? echo $moveRight; ?>">
				<img src="<? echo addslashes(base64_decode($iFv[0])); ?>" <? echo $widthTotal; ?> <? echo $heightTotal; ?> />
				</div>
