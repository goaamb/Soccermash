<?
				...
				////////player text///////////////
					...
					$kind='no';
					if(!empty($_POST['agfifa'])){
						$kindIdProfile="AND ( profileId=13 ";
						$kind='si';
					}	
						
					if(!empty($_POST['aguefa'])){
						if($kind=='si'){
							$andOr='OR';
						}else{
							$andOr='AND (';
						}
						$kindIdProfile=$kindIdProfile. "$andOr profileId=14 ";
						$kind='si';
					}	
					
					if(!empty($_POST['aauth'])){
						if($kind=='si'){
							$andOr='OR';
						}else{
							$andOr='AND (';
						}
						$kindIdProfile=$kindIdProfile. "$andOr profileId=15 ";
						$kind='si';
					}	
						
					
					if($kind=='no'){
						$kindIdProfile="AND ( profileId BETWEEN 13 AND 15";
					}
					
					
					//ctry//
					if(!empty($_POST['countrySearch18'])){
						$_POST['countrySearch17']=$_POST['countrySearch18'];
					}
					
					if(!empty($_POST['countrySearch17'])){
						$countryName="AND countryName='".addslashes($_POST['countrySearch17'])."'";
					}else{
						$countryName='';
					}
					...
					if(!empty($_POST['agfifa'])){
						$aVar[0]=addslashes($_POST['agfifa']);
					}else{
						$aVar[0]='';
					}
					
					if(!empty($_POST['aguefa'])){
						$aVar[1]=addslashes($_POST['aguefa']);
					}else{
						$aVar[1]='';
					}
					
					if(!empty($_POST['aauth'])){
						$aVar[2]=addslashes($_POST['aauth']);
					}else{
						$aVar[2]='';
					}
					
					if(!empty($_POST['countrySearch17'])){
						$aVar[3]=addslashes($_POST['countrySearch17']);
					}else{
						$aVar[3]='';
					}
					
					if(!empty($_POST['ageSearch'])){
						$aVar[4]=addslashes($_POST['ageSearch']);
					}else{
						$aVar[4]='';
					}
					...
					if(!empty($_POST['storeSelectADVProfile'])){
						$aVar[6]=addslashes($_POST['storeSelectADVProfile']);
					}else{
						$aVar[6]='';
					}
			...
			if(!empty($_POST['agfifa'])){
				$v3="<input type='hidden' name='agfifa' value='".$_POST['agfifa']."' />";
			}else{
				$v3="";
			}
			if(!empty($_POST['aguefa'])){
				$v4="<input type='hidden' name='aguefa' value='".$_POST['aguefa']."' />";
			}else{
				$v4="";
			}
			if(!empty($_POST['aauth'])){
				$v5="<input type='hidden' name='aauth' value='".$_POST['aauth']."' />";
			}else{
				$v5="";
			}
			if(!empty($_POST['sexito01'])){
