<?
					...
					//////filters for profile/////
					$kind='no';
					if(!empty($_POST['sptdir'])){
						$kindIdProfile="AND ( profileId=18 ";
						$kind='si';
					}	
						
					if(!empty($_POST['tcnsec'])){
						if($kind=='si'){
							$andOr='OR';
						}else{
							$andOr='AND (';
						}
						$kindIdProfile=$kindIdProfile. "$andOr profileId=19 ";
						$kind='si';
					}	
					
										
					if($kind=='no'){
						$kindIdProfile="AND ( profileId BETWEEN 18 AND 19";
					}
					
										
					//ctry//
					if(!empty($_POST['countrySearch11'])){
						$_POST['countrySearch3']=$_POST['countrySearch11'];
					}
					
					if(!empty($_POST['countrySearch3'])){
						$countryName="AND countryName='".addslashes($_POST['countrySearch3'])."'";
					}else{
						$countryName='';
					}
			...
			if(!empty($_POST['countrySearch3'])){
				$v5="<input type='hidden' name='countrySearch3' value='".$_POST['countrySearch3']."' />";
			}else{
				$v5="";
			}
			
			if(!empty($_POST['sptdir'])){
				$v6="<input type='hidden' name='sptdir' value='".$_POST['sptdir']."' />";
			}else{
				$v6="";
			}
			if(!empty($_POST['tcnsec'])){
				$v7="<input type='hidden' name='tcnsec' value='".$_POST['tcnsec']."' />";
			}else{
				$v7="";
			}
			...
			///Previous results///		
			if($page>0){
			$paginado .="<form name='f2' id='f2' target='iframeSearch' method='POST' action='gestion/modulos/home/findAll.php'><input type='hidden' name='pageNum' value='".($page - 1)."' />$v0 $v1 $v2 $v3 $v4 $v5 $v6 $v7</form>";
			$paginado .= '<span><a href=\'javascript:;\' onclick=\'document.getElementById(\"f2\").submit();\'>'.$_IDIOMA->traducir("previous").'</a></span>';
			}
			
			///More results////		
			if($page<$paginas){
			$paginado .="<form name='e3' id='e3' target='iframeSearch' method='POST' action='gestion/modulos/home/findAll.php'><input type='hidden' name='pageNum' value='".($page + 1)."' />$v0 $v1 $v2 $v3 $v4 $v5 $v6 $v7</form>";
