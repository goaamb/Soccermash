<?
				...
				//////filters for Agent profile/////
					...
					$kind='no';
					if(!empty($_POST['nutr'])){
						$kindIdProfile="AND ( profileId=21 ";
						$kind='si';
					}	
						
					if(!empty($_POST['sptmedic'])){
						if($kind=='si'){
							$andOr='OR';
						}else{
							$andOr='AND (';
						}
						$kindIdProfile=$kindIdProfile. "$andOr profileId=20 ";
						$kind='si';
					}	
					
					if(!empty($_POST['mssgt'])){
						if($kind=='si'){
							$andOr='OR';
						}else{
							$andOr='AND (';
						}
						$kindIdProfile=$kindIdProfile. "$andOr profileId=22 ";
						$kind='si';
					}	
					
					if($kind=='no'){
						$kindIdProfile="AND ( profileId BETWEEN 20 AND 22";
					}
					
										
					//ctry//
					if(!empty($_POST['countrySearch10'])){
						$_POST['countrySearch2']=$_POST['countrySearch10'];
					}
					
					if(!empty($_POST['countrySearch2'])){
						$countryName="AND countryName='".addslashes($_POST['countrySearch2'])."'";
					}else{
						$countryName='';
					}
			...
			if(!empty($_POST['countrySearch2'])){
				$v5="<input type='hidden' name='countrySearch2' value='".$_POST['countrySearch2']."' />";
			}else{
				$v5="";
			}
			
			if(!empty($_POST['nutr'])){
				$v6="<input type='hidden' name='nutr' value='".$_POST['nutr']."' />";
			}else{
				$v6="";
			}
			if(!empty($_POST['sptmedic'])){
				$v7="<input type='hidden' name='sptmedic' value='".$_POST['sptmedic']."' />";
			}else{
				$v7="";
			}
			if(!empty($_POST['mssgt'])){
				$v8="<input type='hidden' name='mssgt' value='".$_POST['mssgt']."' />";
			}else{
				$v8="";
			}
