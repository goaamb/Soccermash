...
switch($type){
	case 'update': 
		...
		if(!mysql_num_rows($query)>0){
			...
			$insert="INSERT INTO $tableProfile (idPlayer,description,date) VALUES ('$idUserVisiting','$value','$date')";
			$res=mysql_query($insert) or die('error: '.mysql_error());			
		
		}else{
		
			$update="UPDATE $tableProfile SET description='$value',date='$date' WHERE idPlayer=$idUserVisiting";
...
?>
	...
	<script>
		...
		window.top.window.$("#loadingSaveTd1").hide();
