...
switch($type){
	...
	case 'insert':
		...
		#Publicacion Muro por  Agent-Player
		if(isset($_SESSION["bEditPlayer"]) && ($_SESSION["bEditPlayer"]==true) && ($_SESSION["sNameAgent"]!=false) ){
			$idUserVisiting		= $_SESSION["iPerfilPlayer"];
			$idProfileVisiting	= $_SESSION["iPerfilPlayer"];
			$user_id		= $_SESSION["iIdPlayer"];#usuario que recibe la publicacion
			$user_id_who		= $_SESSION["iIdPlayer"];#usuario que envia la publicacion
...
class Publication{
	...
	function checkComment($table,$id_user,$id_user_profile,$id_coment,$id_user_who_check,$time){
		...
		$sSQL_Select4321=generateSelect('idUserWhoReceiveAComment,idUserWhoMakeComment,comment',$tableReceivedComments5,"id=$id_coment");
		...
		##qMail
			...
			$iUserIdSM=$_SESSION['idUserVisiting'];#idUsuarioReciveEmail
	...
	//($table,$id_user,$id_user_profile,$id_coment,$id_user_who_check,$time);
		...
		#Selecciono el user id que checkea y el user id que recibe el check
		$sSQL_SELECT2=GenerateSelect('user_id_who,user_id,publication',$tableWall,"id=$id_publication");
	...
	function addComment($table,$comment,$idPublication,$idUserWhoMakeComment,$idUserVisiting,$time){
		...
		$sSQL_Select=generateSelect('idUserWhoMakeComment',$table.'ReceivedComments',"idComment=$idPublication");
		if($DB_Result = $this->oDB->Query($sSQL_Select)){
			...
			foreach ($idUerWhoMakeComment as $id){
				...
				if($DB_ResultProfileId = $this->oDB->Query($sSQL_SelectProfileId)){
					while($resSelectProfileId=mysql_fetch_array($DB_ResultProfileId)){
						...
						if($id != $myId){
							...
							$archivo='/templatemail/emailFromWall.tpl';
							$idUserWhoReceiveComment;#idUsuarioReciveEmail
