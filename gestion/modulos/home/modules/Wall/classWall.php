...
switch ($type) {
	...
	case 'insertComment' :
		...
		$idProfileVisiting = $_POST ['value2'];
		}
	...
	case 'deleteComment' :
		...
		$idRow = $_POST ['value'];
		$jug = new Publication ();
		$mlwc= new ModelLoader ( $table . "ReceivedComments" );
		$ml = new ModelLoader ( $table . "WallAlert" );
		if ($mlwc->buscarPorCampo ( array ("id" => $idRow ) ) && $ml->buscarPorCampo(array("id_Comment"=>$mlwc->idcomment))) {
			$ml->eliminar("id_comment");
		}
	...
	case 'deletePublication' :
		...
		$idRow = $_POST ['value'];
		$jug = new Publication ();
		$mlw= new ModelLoader ( $table . "Wall" );
		$ml = new ModelLoader ( $table . "WallAlert" );
		if ($mlw->buscarPorCampo ( array ("id" => $idRow ) ) && $ml->buscarPorCampo(array("id_publication"=>$mlwc->idpublication))) {
			$ml->eliminar("id_publication");
		}
...
class Publication {
	...
	function deleteComment($tableProfile, $idRow, $idUserVisiting) {
		...
		$sSQL_Delete = "DELETE FROM $tableProfile WHERE (idUserWhoReceiveAComment=$idUserVisiting or idUserWhoMakeComment=$idUserVisiting) AND id=$idRow";
		//var_dump($sSQL_Delete);
		//die('asd');
