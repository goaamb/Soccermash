...
$sqlBase="SELECT * FROM $tableProfile WHERE user_id=$agdate and not user_id_who in ($selectNoActives)";
$asd=$oDB->query("$sqlBase ORDER BY ID DESC LIMIT $inicio,$showOnly");

$paginator=$oDBpag->query($sqlBase);
...
echo '<div style="'.$moveLeft.'"><img src="photoGeneral/small/small_'.$row2['photo'].'" title="'.$row2['name']," ",$row2['lastName'].'" onClick="location.href=\'/'.$_IDIOMA->traducir("user").'/'.$row2['id'].'-'.Utilidades::normalizarTexto($row2["name"]." ".$row2["lastName"]).'\';" alt="#" /></div>';
?>
     </div><!--photoSpeaker-->
     <div class="mainContent">
                     ...
                     <?php echo '<div class="name" onClick="location.href=\'/'.$_IDIOMA->traducir("user").'/'.$row2['id'].'-'.Utilidades::normalizarTexto($row2["name"]." ".$row2["lastName"]).'\';">'; ?>
...
<?php
echo htmlentities(utf8_decode($row['publication']));
?>
                      </div><!--END WROTTE-->
...
?></span>
                     	<span class="addComment"><a href="javascript:;" onClick="javascript:AddCommentAg(<?php echo $row['id']; ?>);" title="<?php print $_IDIOMA->traducir('Add a comment');?>"> <?php print $_IDIOMA->traducir('Comment');?> |</a></span>
							...
							$usersCheck=NULL;
							 ...
							 $valOfAll=NULL;
								   ...
								   $asd70=$oDB->query("SELECT * FROM $tableProfilePubComChecks WHERE id_publication=$commentForRow and not id_user_who_check in($selectNoActives) ");
								...
								<?php $idForChecksUncheks=$row['id']; ?>
								<img src="<?php echo ($you) ? "img/tick.png" : "img/tick_off.png" ; ?>" <?php echo ($you) ? "class='hoverUnCheckPublicationsAg' onClick='unCheckThisPublicationsag($idForChecksUncheks);' title='Uncheck this publication'" : "class='hoverWhitjQueryAg' onClick='javascript:checkThisPublicationag($idForChecksUncheks);' title='". $_IDIOMA->traducir('Check this publication')."'" ; ?> id="<?php echo $row['id']; ?>" />
							...
							<?php
								...
								echo '<p><a href="/'.$_IDIOMA->traducir("user").'/'.$_SESSION['iSMuIdKey'].'-'.Utilidades::normalizarTexto($meName." ".$meLastName).'" class="linkWall" title="'.$_IDIOMA->traducir("See this profile").'">'.$_IDIOMA->traducir("You").'</a> '.$_IDIOMA->traducir("checked it.").'</p>';
								echo '</div>';
							}
							...
							if(($count >= 3) AND ($you)){
								...
								echo '<div class="whoCheck">';
								echo '<span class="imgCheck" title="'.$_IDIOMA->traducir("Who checked it?").'"></span>';
								echo '<p><a href="/'.$_IDIOMA->traducir("user").'/'.$_SESSION["iSMuIdKey"].'-'.Utilidades::normalizarTexto($meName." ".$meLastName).'" class="linkWall" title="'.$_IDIOMA->traducir("See this profile").'">'.$_IDIOMA->traducir("You").'</a> '.$_IDIOMA->traducir("and").' <a class="linkWall" title="'.$_IDIOMA->traducir("See this profile").'">';
					...
					$oDB58->connect();
					$idComment=$row['id'];
					$asd58=$oDB->query("SELECT * FROM $tableProfileComments WHERE idUserWhoReceiveAComment=$agdate and idComment=$idComment and not idUserWhoMakeComment in ($selectNoActives) ORDER BY ID ASC");
	...
	echo '<div style="'.$moveLeft.'"><img src="photoGeneral/small/small_'.$photoWhoMakeComment.'" onClick="location.href=\'/'.$_IDIOMA->traducir("user").'/'.$idWhoMakeComment.'-'.Utilidades::normalizarTexto($nameWhoMakeComment." ".$lastNameWhoMakeComment).'\'" title="'.$nameWhoMakeComment.' '.$lastNameWhoMakeComment.'" alt="#" /></div>';
					?>
						  ...
						  <div class="mainContent mCSUB">
							...
							<?php echo '<div class="name" onClick="location.href=\'/'.$_IDIOMA->traducir("user").'/'.$idWhoMakeComment.'-'.Utilidades::normalizarTexto($nameWhoMakeComment." ".$lastNameWhoMakeComment).'\'" >  '.$nameWhoMakeComment.' '.$lastNameWhoMakeComment.'</div>'; ?>
							 
							<div class="wrotte <?php echo $row58['id']; ?>" value="<?php echo $row58['comment']; ?>"> 
							   <?php echo htmlentities(utf8_decode($row58['comment'])); ?>
							</div>
							<div class="commenTools">
							 ...
							 $valOfAll2=NULL;
								   ...
								   $asd60=$oDB->query("SELECT * FROM $tableProfilePubComChecks WHERE id_coment=$commentForRow and not id_user_who_check in($selectNoActives)");
						...
						<?php 
							...
							if(($count==1) AND ($you)){
								...
								echo '<div class="whoCheck">';
								echo '<span class="imgCheck" title="'.$_IDIOMA->traducir("Who checked it?").'"></span>';
								echo '<p><a href="/'.$_IDIOMA->traducir("user").'/'.$_SESSION['iSMuIdKey'].'-'.Utilidades::normalizarTexto($meName." ".$meLastName).'"
