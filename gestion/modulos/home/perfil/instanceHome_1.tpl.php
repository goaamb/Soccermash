...
*/
if(isset($aUsuario["myAgent"])&&$aUsuario["myAgent"]){
	$mlagent=new ModelLoader("ax_generalRegister");
	if($mlagent->buscarPorCampo(array("id"=>$aUsuario["myAgent"]))){
...
<!-- END LAST CONTRACT DATE -->		
				...
				<?php if($editingProfile){ ?>
                  ...
                  <?php } ?>
				   ...
				   <?php if($editingProfile){ ?>
				  	<li>
				  		...
				  		<!-- para agente -->
						<?php echo $sAgentName;?>
						<?php if(isset($_SESSION["sNameAgent"])&&($_SESSION["sNameAgent"]!=false)){?>
						<a class="goAgent" href="/<?php print $_IDIOMA->traducir("user")."/".$aUsuario["myAgent"].'-'.Utilidades::normalizarTexto($sAgentName);?>"> <?php print $_IDIOMA->traducir("Go to the agent's profile"); ?></a>
                  ...
                  <?php }else{ ?>
				  	<li>
				  		...
				  		<?php echo $sAgentName;?>
						<!-- para agente -->
						<?php if(isset($aUsuario["myAgent"])&&($aUsuario["myAgent"])){?>
						<a class="goAgent" href="/<?php print $_IDIOMA->traducir("user")."/".$aUsuario["myAgent"].'-'.Utilidades::normalizarTexto($sAgentName);?>">-> <?php print $_IDIOMA->traducir("Go to the agent's profile"); ?></a>
						<?php }?>
						<!-- fin para agente -->
				  	</li>
				  ...
				  <?php if($editingProfile){ ?>
				  	<li>
						<?php if(isset($_SESSION["sNameAgent"]) && (isset($_SESSION["bDeleteAgent"])&& $_SESSION["bDeleteAgent"]!=false) ){?>
						<a class="remAgent" href="#" onclick="removeAgent();return false;"><?php print $_IDIOMA->traducir("Remove agent"); ?></a>
						<?php }?>
						<!-- fin para agente -->
				  	</li>
                  <?php }?>
                ...
                <td id="ctrTD" width="280" valign="top" style="padding-right: 10px;">
				 ...
				 <span><?php print $_IDIOMA->traducir("Another Passport/s"); ?>:&nbsp;</span>
               	...
               	<?php if($editingProfile){?>
            		<input type="submit" id="divGif" class="saveEditingP ui-button ui-widget ui-state-default ui-corner-all" role="button" value="<?php print ($_IDIOMA->traducir("Update"));?>"/>
           		<?php }	?>
                </td>
