...
<div style="margin-top:15px;"><a href="javascript:;" onclick="$('#loadComment').html('');"><? print $_IDIOMA->traducir('Close'); ?></a></div>
	...
	require_once($_SERVER['DOCUMENT_ROOT'].$dir.'/gestion/modulos/video/comment/videoCommentClass.php');
		...
		foreach($registros as $registro){
			...
			if($editProf==true && $videoOwner==$_SESSION["iSMuIdKey"]){
				...
				$del='<div class="posDelComment"><a title="delete" href="javascript:;" onclick="delComment(iCpv'.$n.'); $(\'#com'.$registro->id.'\').hide();"><div title="'.$_IDIOMA->traducir("Delete").'" class="delMulti" id="delComment"></div></a></div>';
			}
		...
		////////////leave your comment/////////////
		echo'
		<div style="margin-top:15px; margin-bottom:10px;margin-right: 10px;">'.$_IDIOMA->traducir("Leave your comment").':</div> 
		...
		<?
		 echo '
			...
			<div style="margin-right:8px; margin-top:8px;"><a href="javascript:;" onclick="beforeComment();" class="ui-button ui-widget ui-state-default ui-corner-all">'.$_IDIOMA->traducir("Send").'</a></div>
