...
if (isset ( $_POST ["__accion"] )) {
	switch ($_POST ["__accion"]) {
		case "ingresarAnuncio" :
			if (isset ( $_FILES ) && isset ( $_FILES ["imagen"] ) && is_file ( $_FILES ["imagen"] ["tmp_name"] )) {
				$file = $_FILES ["imagen"];
				...
				if (array_search ( strtolower ( $file ["type"] ), $validmime ) !== false) {
					...
					if (image) {
						...
						switch ($tipo) {
							case "png" :
								imagepng ( $imagex, "goaamb/images/publi/full/$rand.$ext", 100 );
								break;
							case "gif" :
						...
						switch ($tipo) {
							case "png" :
								imagepng ( $imagex, "goaamb/images/publi/thumb/$rand.$ext", 100 );
								break;
							case "gif" :
						...
						$json->add ( "imgdir", "'$rand.$ext'" );
					}
				}
			}
			$json->add ( "url", "'" . $_POST ["url"] . "'" );
			$json->add ( "titulo", "'" . $_POST ["titulo"] . "'" );
			$json->add ( "texto", "'" . $_POST ["texto"] . "'" );
			break;
